...
describe("Cost and Export cards", () => {
  ...
  test("fills missing cost slots with zero values", () => {
    ...
    expect(chartData[0]).toEqual({ slot: 0, time: "00:00", kwh: 0, cost: 0, costCumulative: 0 });
    expect(chartData[1]).toEqual({ slot: 1, time: "00:15", kwh: 0.18, cost: 0.34, costCumulative: 0.34 });
    expect(chartData[2]).toEqual({ slot: 2, time: "00:30", kwh: 0, cost: 0, costCumulative: 0.34 });
    expect(chartData[3]).toEqual({ slot: 3, time: "00:45", kwh: 0, cost: 0, costCumulative: 0.34 });
  });
