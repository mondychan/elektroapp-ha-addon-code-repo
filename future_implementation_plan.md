...
### 1.1 Rozpad monolitu `app_service.py` [HOTOVO]

Logika byla úspěšně přesunuta do servisních modulů v `services/`.
...
### 1.2 Zmenšení `App.js` [HOTOVO]

`App.js` byl rozdělen na komponenty v `components/` a `pages/` a migrován na TypeScript (`App.tsx`).
...
### 2.1 Notifikace a alerty za cenové prahy [HOTOVO]

Implementováno pod `AlertsService` a zobrazeno v UI dashboardu.
...
### 2.2 Srovnání s předchozím dnem/týdnem [HOTOVO]

Implementováno pod `ComparisonService` a zobrazeno v Dashboardu.
...
### 2.3 Export dat (CSV) [HOTOVO]

CSV export měsíčních dat je funkční ze souhrnu i vyúčtování. (PDF stále budoucí).
...
### 3.2 Animované přechody mezi stránkami [HOTOVO]

Přepínání mezi Overview ↔ Detail je nyní plynulé díky `framer-motion`.

---

### 3.3 Dark mode vylepšení [HOTOVO]
...
### 3.5 Lepší mobilní navigace [HOTOVO]
...
### 4.3 Backend — batch endpoint pro dashboard data [HOTOVO]

Frontend dnes stahuje ~8 API volání při načtení dashboardu. Nový batch endpoint `POST /api/dashboard-snapshot` vrací vše v jednom HTTP požadavku.
...
| Fáze | Úkoly | Priorita | Stav |
|---|---|---|---|
| **Fáze 1** | 1.1 Rozpad `app_service.py`, 1.2 Rozpad `App.js`, 1.3 Cache refaktor | Kritická | ✅ Hotovo |
| **Fáze 2** | 3.3 Dark mode auto, 3.5 Bottom nav, 3.2 Animace | Vysoká | ✅ Hotovo |
| **Fáze 3** | 2.1 Cenové alerty, 2.2 Srovnání dní, 4.1 Paralelizace backendu | Střední | ✅ Hotovo |
| **Fáze 4** | 2.3 CSV export, 4.3 Batch endpoint, PND Backfill (31 dní) | Střední | ✅ Hotovo |
| **Fáze 5** | 5.1 Testy, 5.2 TypeScript migrace, 6.1 Monitoring | Dlouhodobá | 🔄 Probíhá |
| **Fáze 6** | 2.4 Solární dashboard (rozšířený), 2.5 Doporučení, 3.4 Tabulky sort | Nice-to-have | ⏳ Čeká |
